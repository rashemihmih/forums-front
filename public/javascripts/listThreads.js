...
if (threads.content.length === 0) {
  ...
  if (adses.code === 0) {
        threads.content.forEach(thread => {
            ...
            html += `<p id="${thread._id}"><a href="${'/thread/' + thread._id + '/page1'}">${thread.title}${thread.title}</a><button class="button button-clear" onclick="deleteT('${thread._id}');">Удалить</button></p>\n`;});
    } else {
      threads.content.forEach(thread => {
          let title = thread.title;
